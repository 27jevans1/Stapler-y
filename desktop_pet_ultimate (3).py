...
# ==================== AI CONFIGURATION ====================
# To enable AI chat, you need an Anthropic API key
# Get one at: https://console.anthropic.com/settings/keys
# Then paste it below:

ANTHROPIC_API_KEY = ""  # PUT YOUR API KEY HERE between the quotes
...
class DesktopPet:
    ...
    def create_sleep_sprites(self):
        ...
        for i in range(3):
            ...
            # Base part
            draw.rectangle([30, 85, 95, 100], fill='#808080', outline='#404040', width=2)
            draw.rectangle([35, 87, 90, 98], fill='#505050', outline='#303030', width=1)
    ...
    def show_chat_dialog(self):
        ...
        if ANTHROPIC_API_KEY and ANTHROPIC_API_KEY != "":
            self.add_chat_message("Stapler", "Hi! I'm a smart stapler! Ask me anything! 📎", "#4A90E2")
        else:
            self.add_chat_message("Stapler", 
                "⚠️ AI not configured yet!\n\n" +
                "To enable me:\n" +
                "1. Get API key: https://console.anthropic.com/settings/keys\n" +
                "2. Open desktop_pet_ultimate.py in text editor\n" +
                "3. Find ANTHROPIC_API_KEY at top\n" +
                "4. Paste your key between quotes\n" +
                "5. Save & restart!\n\n" +
                "Until then, I can't answer questions. 📎", 
                "#FF8C00")
    ...
    def get_ai_response(self, question):
        ...
        if not ANTHROPIC_API_KEY or ANTHROPIC_API_KEY == "":
            self.add_chat_message("Stapler", 
                "❌ No API key configured! To enable AI chat:\n" +
                "1. Get an API key from https://console.anthropic.com/settings/keys\n" +
                "2. Open the .py file in a text editor\n" +
                "3. Find ANTHROPIC_API_KEY at the top\n" +
        ...
        try:
            ...
            # Create persona for the stapler
            system_prompt = """You are a helpful, friendly desktop stapler that has come to life! 
...
Use emojis sparingly (mostly 📎)."""
            ...
            with urllib.request.urlopen(req, timeout=15) as response:
                ...
                if 'content' in result and len(result['content']) > 0:
                    ...
                    self.add_chat_message("Stapler", "Hmm, I couldn't think of a response! 📎", "#E24A4A")
                    self.state = "idle"
        
        except urllib.error.HTTPError as e:
            error_msg = e.read().decode('utf-8') if e.fp else str(e)
            if e.code == 401:
                self.add_chat_message("Stapler", "❌ Invalid API key! Check your key at the top of the .py file.", "#E24A4A")
            elif e.code == 429:
                self.add_chat_message("Stapler", "⏳ Rate limit hit! Wait a moment and try again. 📎", "#E24A4A")
            else:
                self.add_chat_message("Stapler", f"❌ API Error {e.code}: Check your API key and internet connection!", "#E24A4A")
            self.state = "idle"
        except urllib.error.URLError as e:
            self.add_chat_message("Stapler", "❌ Can't connect! Check your internet connection. 📎", "#E24A4A")
            self.state = "idle"
        except Exception as e:
            self.add_chat_message("Stapler", f"❌ Error: {str(e)[:100]} 📎", "#E24A4A")
            self.state = "idle"
        finally:
            self.is_thinking = False
